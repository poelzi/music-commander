...
- Key bug fixes: `TRACK_NUM` → `TRACKNUMBER` tag mapping (was `TRACKNU02MBER`), encoding fallback chain, filesystem-safe filename generation
- Parser should return dataclass instances (`CueSheet`, `CueTrack`) not raw dicts
- The `_timestr_to_samples` method is correct (44100Hz, 75 frames/sec) — preserve it
...
### Risks & Mitigations
- shntool package name may differ across nixpkgs versions → verify with `nix search nixpkgs shntool`.
- Cue sheet encoding edge cases → comprehensive test fixtures needed.
...
- Tag mapping: PERFORMER→ARTIST, global TITLE→ALBUM, track TITLE→TITLE, TRACK_NUM→TRACKNUMBER, GENRE, DATE, SONGWRITER, ISRC, DISCID
...
### Risks & Mitigations
- shntool may not support APE/WV without helper programs → ffmpeg fallback handles this.
- metaflac ReplayGain requires all album tracks to exist → run after all tracks are split.

---

## Work Package WP03: CLI Command Group & Split Command (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Command auto-discovery requires `cli` attribute in `__init__.py` — follow established pattern exactly.
- Recursive walks on large collections may produce lots of output → use `verbose()` for per-file details.
...
### Risks & Mitigations
- ffmpeg timestamp precision may differ from shntool frame-level precision → acceptable for lossy source formats (APE/WV).
- Rare cue sheet encodings (Shift-JIS) not in default fallback → `--encoding` flag covers this.
