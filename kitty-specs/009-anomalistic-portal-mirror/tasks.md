...
## Work Package WP01: Foundation – Shared Matching, Config, and Cache Models (Priority: P0)
...
### Risks & Mitigations
- Matcher extraction may break existing bandcamp tests → run full test suite after extraction.
...
### Risks & Mitigations
- Portal API may change or be rate-limited → add retry logic with backoff for 429/503.
...
### Risks & Mitigations
- HTML structure varies between releases → test against multiple real samples (compilations, singles, no tracklist).
...
### Risks & Mitigations
- `unrar` not in PATH → detect at startup, warn user with installation guidance.
- Corrupt archives → catch extraction errors, mark release as failed, continue with next.
...
### Risks & Mitigations
- ffmpeg errors on specific files → catch per-file, log error, continue with next file.
- Jinja2 template syntax errors → validate pattern at config load time.
...
- T036: If any check matches → skip with message. Priority: URL cache > comment scan > fuzzy match.
...
### Risks & Mitigations
- False positive fuzzy matches → use conservative threshold; log match score for debugging.

---

## Work Package WP07: CLI Command and Integration (Priority: P1) 🎯 MVP
...
### Risks & Mitigations
- Long-running operation → handle KeyboardInterrupt gracefully, save progress to cache.
- Partial failures → continue with next release, report failures in summary.
