...
- Join `library` → `track_locations` for file paths
...
### Risks & Mitigations
- Mixxx schema version differences → validate against schema.xml from Mixxx repo
- Large result sets → use generator/iterator pattern for memory efficiency
...
### Risks & Mitigations
- Subprocess hangs → implement timeout and graceful termination
...
### Risks & Mitigations
- Sentinel file accidentally deleted → recreate on next sync
- Clock skew between machines → use UTC timestamps exclusively

---

## Work Package WP04: Core Sync Logic (Priority: P1) 🎯 MVP

**Goal**: Implement the main sync workflow: query Mixxx → filter changes → transform → write to annex.
...
### Risks & Mitigations
- Memory with large libraries → use streaming/generator pattern
- Partial failure → log per-file errors, continue, report summary
...
### Risks & Mitigations
- Path validation edge cases → test with symlinks, relative paths, absolute paths
...
### Risks & Mitigations
- Documentation drift → keep docs close to implementation
...
```
WP01 (Mixxx Queries) ──┬──> WP04 (Core Sync) ──> WP05 (CLI) ──> WP07 (Polish)
                       │                              │
WP02 (Annex Batch) ────┤                              │
                       │                              │
WP03 (Sync State) ─────┘                              │
...
- **Sequence**: WP01+WP02 (parallel) → WP03 → WP04 → WP05 → WP06+WP07 (parallel)
- **Parallelization**: WP01 and WP02 can proceed in parallel; WP06 and WP07 can proceed in parallel
- **MVP Scope**: WP01 → WP02 → WP03 → WP04 → WP05 (basic sync without crates)
