    ...
    if [[ ! "$current_branch" =~ ^[0-9]{3}- ]]; then
      if latest_worktree=$(find_latest_feature_worktree "$repo_root" 2>/dev/null); then
        if [[ -d "$latest_worktree" ]]; then
          >&2 echo "[spec-kitty] Auto-running merge inside $latest_worktree (current branch: $current_branch)"
          (
            cd "$latest_worktree" && \
...
/nix/store/8pd3b2rxdjvzmqb00n0ik3a006dh65q0-spec-kitty-cli-0.9.4/bin/spec-kitty-python "$PY_HELPER" merge "$@"
